                ...
                <div class="list-box">
                    ...
                    <div v-if="hometype==3&&wallet_changes==true">
                        <ul class="center-ul" v-for="(item,index) in datas" :key="index">
                            ...
                            <li class="li2 color1" @click="item_search(item.fromAddress)">{{filterFun(item.fromAddress)}}</li>
                            <li class="li3 color1" @click="item_search(item.toAddress)">{{filterFun(item.toAddress)}}</li>
                            <li class="li4">{{item.InOrOut}}</li>
                            <li class="li5">{{comdify((item.value / Math.pow(10, 18)).toFixed(8))}}</li>
                            <!-- <li class="li6" >{{comdify(item.value / Math.pow(10, 18).toFixed(8))}}</li>  -->
                        </ul>
                    </div>
                    <!--搜索钱包地址 挖块统计-->
                    <div v-if="hometype==3&&wallet_changes==false">
                        <ul class="center-ul" v-for="(item,index) in datas" :key="index">
                            <li class="li0 color1" @click="item_search(item.number)">{{comdify(item.number)}}</li>
                            <li class="li1">{{timestampToTime(item.timestamp)}}</li>
                            <li class="li2">
                                <img src="../assets/img/KIR.png" alt="" v-if="item.minerFiner==1">
                                <img src="../assets/img/NA.png" alt="" v-if="item.minerFiner!==1">
                            </li>
                            <li class="li3 color1" @click="item_search(item.number)">{{item.hash}}</li>
                            <li class="li4">{{changpow(item.difficulty)}}</li>
                            <li class="li5">{{comdify(item.transactionNumber)}}</li>
                            <li class="li6">{{format(item.reward,8)||0}}</li>
                        </ul>
                    </div>
                </div>
                <!--分页-->
...
export default {
    ...
    methods: {
        wallet_change(val) {
            ...
            if (this.wallet_changes) {
                ...
                _that.$set(_that.totalSize, 0, _that.homedatalist.TxCount)
                ...
                console.log('Block111')
                this.datas = []
                console.log(_that.homedatalist)

                _that.$set(_that.totalSize, 0, _that.homedatalist.blockCount)
                ...
                console.log(this.datas, 'ssssss1111')
        ...
        handleSizeChange(val) {
            if (this.hometype == 3) {
                ...
                this.$nextTick(() => {
                    this.tosearch(this.wallet_changes)
                })
                return
            }
            this.blockListObj.pageSize = val;
    ...
    watch: {
        ...
        //         // this.wallet_change()
        //         console.log(this.wallet_changes)
        //     }
        // }
    }
}
