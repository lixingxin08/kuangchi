<!--<template>-->
  <!--<div class="login">-->
    <!--<div class="parent-box">-->
      <!--<div class="login-box">-->
        <!--<p class="login-p">{{$t("m.login.key1")}}</p>-->
        <!--&lt;!&ndash;<p class="login-text">SIGN IN KIRINMINER</p>&ndash;&gt;-->
        <!--<div class="phone-email"><span class="active" @click="selectRegister(2)">{{$t("m.login.key3")}}</span><span @click="selectRegister(1)">{{$t("m.login.key2")}}</span></div>-->
        <!--<div>-->
          <!--&lt;!&ndash;<p v-show="phoneP" style="color: red">{{$t("m.key2")}}</p>&ndash;&gt;-->
          <!--<div  v-show="isPhone" class="username-box" style="position: relative">-->
            ...
            <!--</div>-->
            <!--<span><input-->
              <!--type="text" id="box" v-model="loginObj.mobile" :placeholder="$t('m.register.key15')"></span>-->
          <!--</div>-->
          <!--&lt;!&ndash;<p v-show="emailP" style="color: red">{{$t("m.key3")}}</p>&ndash;&gt;-->
          <!--<div class="username-box" v-show="isEmail">-->
            <!--<img src="../assets/img/emailIcon.png" alt="">-->
            <!--<input type="text" v-model="loginObj.email" :placeholder="$t('m.login.key5')" >-->
          <!--</div>-->
          <!--&lt;!&ndash;<p v-show="passwordP" style="color: red">{{$t("m.key11")}}{{$t("m.key12")}}</p>&ndash;&gt;-->
          <!--<div class="password-box">-->
            <!--<img src="../assets/img/passworIcon.png" alt=""><input type="password" v-model="loginObj.password" :placeholder="$t('m.login.key10')">-->
          <!--</div>-->
          <!--<router-link to="/reset"><p class="forgetPassword">{{$t("m.login.key6")}}</p></router-link>-->
          <!--<button class="login-btn" @click="login(1,$event)" v-show="isPhone">{{$t("m.login.key1")}}</button>-->
          <!--<button class="login-btn" @click="login(2,$event)" v-show="isEmail">{{$t("m.login.key1")}}</button>-->
          <!--<router-link to="/register"><p class="goRegister-p"><span>{{$t("m.login.key7")}}</span><span class="registerText-span">{{$t("m.login.key8")}}</span></p></router-link>-->
        <!--</div>-->
      <!--</div>-->
  ...
  <!--export default {-->
    ...
    <!--methods: {-->
      ...
      <!--login(index,e) {-->
        ...
        <!--if (index === 1) {-->
          <!--if (!this.loginObj.mobile) {-->
            <!--alert(this.$t("m.login.key4"));-->
            <!--return false;-->
          <!--} else {-->
            <!--path = "login";-->
            <!--loginObj.phone = this.loginObj.mobile;-->
            <!--loginObj.countryCode =  this.region;-->
            <!--loginObj.mode =  "phone"-->
            <!--// loginObj.countryCode = this.region;-->
          <!--}-->
        <!--} else if (index === 2) {-->
          <!--if (!this.loginObj.email) {-->
            <!--alert(this.$t("m.login.key5"));-->
            <!--return false;-->
          <!--}-->
          <!--else {-->
            <!--loginObj.email = this.loginObj.email;-->
            <!--loginObj.mode =  "email";-->
            <!--path = "login";-->
          <!--}-->
        <!--}-->
        <!--if (!this.loginObj.password) {-->
          <!--alert(this.$t("m.login.key10"));-->
          <!--return false;-->
        <!--} else {-->
          <!--loginObj.platformName = this.platformName;-->
          <!--loginObj.password = this.loginObj.password;-->
          <!--loginObj = Qs.stringify(loginObj);-->
          <!--var vueThis = this;-->
          <!--btn.text(this.$t("m.login.key11"));-->
          <!--this.$axios({-->
            <!--method: "post",-->
            <!--url: this.baseUrlTwo + path,-->
            <!--data: loginObj,-->
            <!--withCredentials:true-->
          <!--}).then(function (res) {-->
            <!--if (res.data.code === 1) {-->
              <!--btn.text(vueThis.$t("m.login.key1"));-->
              <!--vueThis.setCookie("username",res.data.data.username);-->
              <!--vueThis.setCookie("token",res.data.data.token);-->
              <!--// vueThis.setCookie("token",res.data.data.token);-->
              <!--vueThis.$message.success(vueThis.$t("m.login.key15"));-->

              <!--if(res.data.data.platformUrl){-->
                <!--vueThis.setCookie("platformUrl",res.data.data.platformUrl);-->
                <!--window.setTimeout(function () {-->
                  <!--window.location.href = "http://"+ res.data.data.platformUrl;-->
                <!--},500);-->

              <!--}else {-->
                <!--// vueThis.$router.push("/account/setting")-->
              <!--}-->
            <!--} else {-->
              <!--btn.text(vueThis.$t("m.login.key1"));-->
              <!--if(res.data.code === 1038){-->
                <!--alert(vueThis.$t("m.login.key16"));-->
              <!--}else if(res.data.code === 1039){-->
                <!--alert(vueThis.$t("m.login.key17"));-->
              <!--}else if(res.data.code === 1040){-->
                <!--alert(vueThis.$t("m.login.key18"));-->
              <!--}else if(res.data.code === 1041){-->
                <!--alert(vueThis.$t("m.login.key19"));-->
              <!--}else if(res.data.code === 1042){-->
                <!--alert(vueThis.$t("m.login.key20"));-->
              <!--}else if(res.data.code === 1043){-->
                <!--alert(vueThis.$t("m.login.key13"));-->
              <!--}-->
            <!--}-->
          <!--}).catch(function (err) {-->
            <!--btn.text(vueThis.$t("m.login.key1"));-->
