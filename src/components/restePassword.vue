<!--<template>-->
  <!--<div class="login">-->
    <!--<div class="parent-box">-->
      <!--<div class="login-box">-->
        <!--<p class="login-p">{{$t("m.resetPassword.key1")}}</p>-->
        <!--&lt;!&ndash;<p class="login-text">SIGN IN WTCPOOL</p>&ndash;&gt;-->
        <!--<div class="register-center">-->
          <!--<div class="phone-email"><span class="active" @click="selectRegister(2)">{{$t("m.resetPassword.key3")}}</span><span  @click="selectRegister(1)">{{$t("m.resetPassword.key2")}}</span></div>-->
          <!--<p v-show="phoneP" style="color: red">{{$t("m.key2")}}</p>-->
          <!--<div  v-show="isPhone" class="username-box" style="position: relative">-->
            <!--<div class="selected-countries-box"><span style="display: inline-block;width: 100%"  class="dropdown-toggle nation-phone"  data-toggle="dropdown">+86</span>-->
              <!--<div class="phoneQz-box dropdown-menu custom-dropdown-menu">-->
                <!--<ul class="user-msg-ul" id="user-msg-ul">-->
                  <!--<li v-for="item in phoneQz">+<span class="phone-span">{{item.tel}}</span><span v-show="$i18n.locale === 'zh'">{{item.name}}</span><span v-show="$i18n.locale === 'en'">{{item.en}}</span></li>-->
                <!--</ul>-->
              <!--</div>-->
            <!--</div>-->
            <!--<span><input-->
              <!--type="text" id="box" v-model="resetPasswordObj.phone" :placeholder="$t('m.resetPassword.key4')" @blur="IsPhone(resetPasswordObj.phone)"></span>-->
          <!--</div>-->
          <!--<p v-show="emailP" style="color: red">{{$t("m.key3")}}</p>-->
          <!--<div v-show="isEmail" class="username-box email-box">-->
            <!--<input type="email" :placeholder="$t('m.resetPassword.key5')" v-model="resetPasswordObj.email" @blur="vmf(resetPasswordObj.email)">-->
          <!--</div>-->
          <!--<p v-show="passwordP" style="color: red">{{$t("m.key13")}}</p>-->
          <!--<div class="username-box">-->
            <!--<input type="password" :placeholder="$t('m.resetPassword.key8')" v-model="resetPasswordObj.password" @blur="verifyPassword(resetPasswordObj.password)">-->
          <!--</div>-->
          <!--<div class="username-box">-->
            <!--<input type="password" :placeholder="$t('m.resetPassword.key9')" v-model="confirmPassword">-->
          <!--</div>-->
          <!--<div class="username-box getCode-box" v-show="isEmail">-->
            <!--<span>-->
            <!--<input type="text" :placeholder="$t('m.resetPassword.key6')" v-model="resetPasswordObj.code"></span>-->
            <!--<span>-->
            <!--<button class="getCode-btn" @click="getCode(2)">{{$t("m.resetPassword.key7")}}</button></span>-->
          <!--</div>-->
          <!--<div class="username-box getCode-box" v-show="isPhone">-->
            <!--<span>-->
            <!--<input type="text" :placeholder="$t('m.resetPassword.key6')" v-model="resetPasswordObj.code"></span>-->
            <!--<span>-->
            <!--<button class="getCode-btn" @click="getCode(1)">{{$t("m.resetPassword.key7")}}</button></span>-->
          <!--</div>-->
          <!--&lt;!&ndash;<p class="forgetPassword">忘记密码?</p>&ndash;&gt;-->
          <!--<button class="login-btn" v-show="isPhone" @click="resetPassword(1)">{{$t("m.resetPassword.key1")}}</button>-->
          <!--<button class="login-btn" v-show="isEmail" @click="resetPassword(2)">{{$t("m.resetPassword.key1")}}</button>-->
          <!--<router-link to="/login"><p class="goRegister-p">{{$t("m.resetPassword.key10")}}</p></router-link>-->

        <!--</div>-->
      <!--</div>-->
      <!--<div class="background-box">-->
        <!--<img src="../assets/img/loginBeijin2.png" alt="">-->
      <!--</div>-->
    <!--</div>-->
  <!--</div>-->
<!--</template>-->
  ...
  <!--export default {-->
    ...
    <!--methods:{-->
      ...
      <!--selectRegister(way){-->
        ...
        <!--this.resetPasswordObj.phone = "";-->
        <!--this.resetPasswordObj.code = "";-->
        <!--this.resetPasswordObj.password = "";-->
        <!--this.confirmPassword = "";-->
        <!--this.passwordP = false;-->
        <!--this.phoneP = false;-->
        <!--this.emailP = false;-->
        <!--if(way === 1){-->
          <!--this.isPhone = true;-->
        ...
        <!--}else if(way === 2){-->
          ...
          <!--this.isEmail = true;-->
        <!--}-->
        <!--$(".phone-email span").on("click",function () {-->
      ...
      <!--selectCountries(){-->
        <!--let vueThis = this;-->
        <!--$("#user-msg-ul>li").on("click",function () {-->
          <!--var str = ($(this).children(".phone-span").text());-->
          <!--// if(str !== "86"){-->
          <!--//   alert(vueThis.$t("m.key14"))-->
          <!--// }else {-->
      ...
      <!--//校验密码-->
      <!--verifyPassword(str){-->
        <!--if(this.isNumberOr_Letter(str) !== true){-->
          <!--this.passwordP = true-->
        <!--}else {-->
          <!--this.passwordP = false-->
        <!--}-->
      ...
      <!--validateInput(obj,index) {-->
        <!--if(obj){-->
          ...
          <!--var complex = 0;-->
          <!--var complexTwo = 0;-->
          <!--if (regLower.test(obj)) {-->
            <!--++complex;-->
          <!--}-->
          <!--if (regUpper.test(obj)) {-->
            <!--++complexTwo;-->
          <!--}-->
          <!--if (regStr.test(obj)) {-->
            <!--++complex;-->
          <!--}-->
          <!--if (complex < 3 || obj.length < 8) {-->
            <!--// alert("包含大小写字母，数字,特殊字符,长度至少8位");-->
            <!--if(index === 1){-->
              <!--$("#password-box").addClass("active");-->
              <!--$("#password-p").text(this.$t("m.key116_4")).css("color","red")-->
            <!--}else if(index === 2){-->
              <!--return false;-->
            <!--}-->
          ...
          <!--} else {-->
            <!--if(index === 1){-->
              ...
              <!--$("#password-p").text(this.$t("m.key123")).css("color","#fff")-->
            <!--}else if(index === 2){-->
              <!--return true;-->
            <!--}-->
          <!--}-->
      ...
      <!--getCode(index){-->
        ...
        <!--if(index === 1){-->
          <!--if(!this.resetPasswordObj.phone){-->
            <!--alert(this.$t("m.resetPassword.key4"));-->
            <!--return false-->
          <!--}else {-->
            <!--var path = "sendSMSToChangPwd";-->
            <!--getCodeObj.countryCode = this.region;-->
            <!--getCodeObj.mobile = this.resetPasswordObj.phone-->
            <!--// if(this.region !== "86"){-->
            <!--//   getCodeObj.mobile = this.region + this.resetPasswordObj.phone-->
            <!--// }else {-->
            <!--//   getCodeObj.mobile = this.resetPasswordObj.phone-->
            <!--// }-->
          <!--}-->
        <!--}else if(index === 2 ){-->
          <!--if(!this.resetPasswordObj.email){-->
            <!--alert(this.$t("m.resetPassword.key5"));-->
            <!--return false-->
          <!--}else if(this.verEmail(this.resetPasswordObj.email) !== true){-->
            <!--alert(this.$t("m.key3"));-->
            <!--return false;-->
          <!--}else {-->
            <!--var path = "sendEmailToChangePwd";-->
            <!--getCodeObj.email = this.resetPasswordObj.email-->
          <!--}-->
        <!--}-->
        <!--getCodeObj = Qs.stringify(getCodeObj);-->
        <!--var clickDom = $(event.currentTarget);-->
        <!--clickDom.attr("disabled", true).css("cursor", "not-allowed");-->
        <!--this.$axios({-->
          <!--method:"post",-->
          <!--url:this.baseUrl +"v1/user/" + path,-->
          <!--data:getCodeObj,-->
          <!--headers: {-->
        ...
        <!--}).then(function (res) {-->
          <!--if(res.data.code === 200){-->
              <!--vueThis.countDown(clickDom);-->
          <!--}else {-->
            <!--clickDom.removeAttr("disabled").css("cursor", "pointer");-->
            <!--if(res.data.code === 1003 || res.data.code === 1004){-->
              <!--alert(this.$t("m.resetPassword.key11"));-->
            <!--}else  if(res.data.code === 500){-->
             <!--if(index === 1){-->
               <!--alert(this.$t("m.register.key18"))-->
             <!--}else if(index === 2){-->
               <!--alert(this.$t("m.register.key24"))-->
             <!--}-->
            <!--}else if (res.data.code === 501) {-->
              <!--alert(vueThis.$t("m.register.key84"));-->
            <!--} else if (res.data.code === 502) {-->
              <!--alert(vueThis.$t("m.register.key85"));-->
            <!--}-->
        <!--}-->
        <!--}).catch(function (err) {-->
          <!--clickDom.removeAttr("disabled").css("cursor", "pointer");-->
      ...
      <!--resetPassword(way){-->
        ...
        <!--if(way === 1){-->
          <!--if(!this.resetPasswordObj.phone){-->
            <!--alert(this.$t("m.resetPassword.key4"));-->
            <!--return false;-->
          <!--}else if(this.IsPhone(this.resetPasswordObj.phone) !== true){-->
            <!--alert(this.$t("m.key2"));-->
            <!--return false;-->
          <!--} else {-->
            <!--// if(this.region !== "86"){-->
            <!--//   obj.mobile = this.region + this.resetPasswordObj.phone-->
            <!--// }else {-->
            <!--//   obj.mobile = this.resetPasswordObj.phone;-->
            <!--// }-->
            <!--path = "forgetMobilePassword";-->
            <!--obj.mobile = this.resetPasswordObj.phone;-->
        ...
        <!--}else if(way === 2){-->
          ...
          <!--} else {-->
            ...
            <!--obj.email = this.resetPasswordObj.email;-->
          <!--}-->
        <!--}-->
          <!--if(!this.resetPasswordObj.code){-->
            <!--alert(this.$t("m.resetPassword.key6"));-->
          <!--return false;-->
        <!--}else  if(!this.resetPasswordObj.password){-->
            <!--alert(this.$t("m.resetPassword.key15"));-->
          <!--return false;-->
        <!--}else if(this.isNumberOr_Letter(this.resetPasswordObj.password)!== true){-->
            <!--alert(this.$t("m.key13"));-->
            <!--return false;-->
          <!--}else  if(!this.confirmPassword){-->
            <!--alert(this.$t("m.resetPassword.key9"));-->
          <!--return false;-->
        <!--}else  if(this.resetPasswordObj.password !== this.confirmPassword){-->
            <!--alert(this.$t("m.resetPassword.key12"));-->
          <!--return false;-->
        <!--}else {-->
          <!--var vueThis = this;-->
            <!--obj.code = this.resetPasswordObj.code;-->
            <!--obj.password = this.resetPasswordObj.password;-->
          <!--var registerObj = Qs.stringify(obj);-->
          <!--this.$axios({-->
            <!--method:"post",-->
            <!--url:this.baseUrl + "v1/user/" + path,-->
            <!--data:registerObj,-->
          ...
          <!--}).then(function (res) {-->
            <!--if(res.data.code === 200){-->
              <!--vueThis.resetPasswordObj.phone = "";-->
              <!--vueThis.resetPasswordObj.email = "";-->
              <!--vueThis.resetPasswordObj.code = "";-->
              <!--vueThis.resetPasswordObj.password = "";-->
              <!--vueThis.confirmPassword = "";-->
              <!--vueThis.$router.push("/login");-->
              <!--alert(vueThis.$t("m.resetPassword.key13"));-->
            <!--}else {-->
              <!--if(res.data.code === 1002){-->
                <!--alert(vueThis.$t("m.resetPassword.key14"));-->
              <!--}else if(res.data.code === 1001){-->
                <!--alert(vueThis.$t("m.register.key25"));-->
              <!--}else if(res.data.code === 4){-->
                <!--alert(vueThis.$t("m.setting.key36"));-->
              <!--}-->
            <!--}-->
          <!--}).catch(function (err) {-->
          <!--})-->
...
<!--<style scoped>-->
 ...
 <!--}-->
  ...
  <!--}-->
  <!--.active{-->
    <!--border-bottom: 2px solid #e4d494;-->
    <!--font-size: 22px;-->
  <!--}-->
  <!--.activeTwo{-->
